...
public class Field {
    ...
    /**
     * This method moves all lines down to replace deleted one
     * New empty line is created on the top
     */
    private void moveLines(int[][] matrix, int index) {
        for (int i = index; i >= 0; i--) {
            if (matrix[i] != null) continue;
            if (i > 0) {
                //Swap current null row with row above
                matrix[i] = matrix[i - 1];
                matrix[i - 1] = null;
            }
            if (i == 0) matrix[i] = new int[width];
        }
    }

    /**
     * This method checks if all fields in a row are equal to 1
     */
